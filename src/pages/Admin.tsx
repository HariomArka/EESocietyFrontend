...
export default function Admin() {
  ...
  const [deleteConfirm, setDeleteConfirm] = useState<{ type: 'blog' | 'project'; id: string; name: string } | null>(null);
  const [saving, setSaving] = useState(false);

  useEffect(() => { if (token) { loadBlogs(); loadProjects(); } }, [token]);
  ...
  const handleLogout = () => { setToken(null); localStorage.removeItem('adminToken'); setBlogs([]); setProjects([]); };
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      else { await deleteProject(deleteConfirm.id, token!); loadProjects(); }
    ...
    <div className="min-h-screen bg-black text-white">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex gap-1 bg-white/5 border border-white/8 p-1 rounded-xl w-fit mb-6">
          {(['blogs', 'projects'] as const).map(tab => (
            <button key={tab} onClick={() => setActiveTab(tab)}
              className={`px-5 py-2 rounded-lg text-sm font-semibold capitalize transition-all ${activeTab === tab ? 'bg-blue-600 text-white shadow-md shadow-blue-600/30' : 'text-gray-500 hover:text-gray-300'}`}>
              {tab === 'blogs' ? `Blogs (${blogs.length})` : `Projects (${projects.length})`}
            </button>
          ))}
        </div>
